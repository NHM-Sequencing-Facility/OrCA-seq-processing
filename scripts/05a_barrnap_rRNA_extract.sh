...
# Run barrnap
barrnap -k euk --threads 1 --incseq \
...
if [ -f "${temp_dir}/${identifier}_euk.fa" ]; then
    ...
    count_18S_original=$(seqkit grep -r -p "18S_rRNA" "${temp_dir}/${identifier}_euk.fa" | grep -c "^>" 2>/dev/null; true)
    count_28S_original=$(seqkit grep -r -p "28S_rRNA" "${temp_dir}/${identifier}_euk.fa" | grep -c "^>" 2>/dev/null; true)
